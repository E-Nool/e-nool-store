...
export const getProductImage = (imageURL: string | null): string => {
  if (!imageURL) return "images/LandingPage/TrendingReads5.png"
  // const newBaseURL = "https://enool.orcans.site/"
  const newBaseURL = process.env.NEXT_PUBLIC_MEDUSA_BACKEND_URL || 'http://localhost';

  // Replace the base URL only if the string contains "http://localhost:9000/"
  if (imageURL?.includes("http://localhost:9000/")) {
    return imageURL.replace("http://localhost:9000/", newBaseURL)
  }
  return imageURL
}
